...
void FfmpegPlayer::startVideoProcess()
{
    ...
    // Przełącznik logowania stderr ffmpeg do konsoli aplikacji (qDebug()).
    // Ustaw na `false`, żeby całkowicie wyciszyć te wpisy - np. gdy
    // "-loglevel error" powyżej i tak już nic nie zgłasza, a i tak wolisz
    // pustą konsolę. UWAGA: to jest DRUGIE, NIEZALEŻNE źródło logów obok
    // samego ffmpeg (patrz komentarz przy "-loglevel" w args poniżej) -
    // przy diagnozowaniu problemów ze strumieniem warto to na chwilę
    // włączyć z powrotem (np. właśnie dzięki temu udało się namierzyć
    // wcześniej błędy "Option rtsp_transport not found" i "404 Not Found").
    static const bool LOG_FFMPEG_STDERR = false;

    connect(videoProcess, &QProcess::readyReadStandardError, this, [this](){
        if (LOG_FFMPEG_STDERR && videoProcess)
            qDebug() << "FFmpeg:" << videoProcess->readAllStandardError().left(300);
        else if (videoProcess)
            videoProcess->readAllStandardError(); // i tak trzeba opróżnić bufor
    });
    ...
    QStringList args = {
        "-hide_banner",
        // "-loglevel" kontroluje ILE ffmpeg w ogóle wypisuje:
        //   "quiet"  - zero logów, nawet błędów (odradzane - nic nie
        //              zobaczysz gdy strumień faktycznie padnie, np. jak
        //              przy diagnozowaniu "Option rtsp_transport not
        //              found" albo "404 Not Found" wcześniej w tej
        //              rozmowie)
        //   "panic"  - tylko błędy, po których ffmpeg się wywala
        //   "error"  - jak wyżej + inne błędy nie zawsze fatalne (zalecane)
        //   "warning"- jeszcze więcej (domyślne dla wielu wersji to "info")
        "-loglevel",      "error",
        "-nostats",
        "-y",
